...
```json
{
  ...
  "thumbnail_key": "videos/thumbnails/video_789.jpg",
  "preview_key": "videos/previews/video_789.mp4",
  "hls_master_key": "videos/hls/video_789/master.m3u8",
...
`bucket` is still accepted as a backwards-compatible shorthand for both source and output buckets. `source_bucket` defaults to `S3_BUCKET`; `output_bucket` defaults to `VIDEO_OUTPUT_BUCKET` and then `S3_BUCKET`. `output_prefix` is optional and defaults to `videos/<asset_id>`. The worker writes HLS playlists, `thumbnail.jpg`, and `preview.mp4` under `output_prefix`; explicit `thumbnail_key`, `preview_key`, and `hls_master_key` are accepted for producer/DB compatibility.
