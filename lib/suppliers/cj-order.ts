 ...
 * Maps Shopify orders → CJ orders for automatic fulfillment
 * 
 * Flow: Shopify webhook → extract SKU (ACV-{cjPid}) → get CJ vid → create CJ order
 */
...
/**
 * Extract CJ Product ID from Shopify SKU
...
export async function createCJOrder(params: {
  ...
  try {
    ...
    const body = {
      orderNumber: `SHOP-${params.shopifyOrderId}`,
    ...
    console.log(`[CJ Order] Creating order for Shopify #${params.shopifyOrderId}...`);
...
/**
 * Full fulfillment flow: Shopify order → CJ order
 * 1. Extract CJ PID from Shopify SKU
 * 2. Get VID from CJ
 * 3. Create CJ order
 */
export async function fulfillFromShopify(shopifyOrder: {
  orderId: string;
  ...
  console.log(`[Fulfillment] Processing Shopify order #${shopifyOrder.orderId}...`);
  ...
  return createCJOrder({
    ...
    shippingName: shopifyOrder.shipping.name,
    shippingPhone: shopifyOrder.shipping.phone,
    shippingEmail: shopifyOrder.shipping.email,
    shippingAddress: shopifyOrder.shipping.address1,
    shippingCity: shopifyOrder.shipping.city,
    shippingProvince: shopifyOrder.shipping.province,
    shippingZip: shopifyOrder.shipping.zip,
    shippingCountryCode: shopifyOrder.shipping.countryCode,
    products: cjProducts,
  });
}
