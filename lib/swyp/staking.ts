...
export async function processMaturedStakes(limit = 50): Promise<{ processed: number; bonusesPaid: number }> {
  ...
  for (const s of due) {
    ...
    // 1. Principalul înapoi (idempotent pe stake id).
    await swypTransfer({
      from: { pool: "staking" },
      to: { userId: s.user_id },
    ...
    // 2. Bonusul, în limita bugetului de surplus.
    const nominal = nominalBonusUnits(principal, s.apy_bps, s.term_months);
    const bonus = nominal <= budget ? nominal : 0n; // sub buget: plată integrală; altfel amânat
    if (bonus > 0n) {
      await swypTransfer({
        from: { pool: "rewards" },
        to: { userId: s.user_id },
    ...
    await dbQuery(
      `UPDATE swyp_stakes SET status = 'matured', closed_at = now(), bonus_units = $2 WHERE id = $1 AND status = 'active'`,
      [s.id, bonus.toString()],
    );
    processed++;

    if (bonus === 0n && nominal > 0n) {
      // buget epuizat: lăsăm stake-ul marcat matured cu bonus 0 DOAR dacă am
      // decis să nu-l plătim deloc; aici alegem amânarea → revert la active.
      await dbQuery(
        `UPDATE swyp_stakes SET status = 'active', closed_at = NULL, bonus_units = NULL
          WHERE id = $1 AND bonus_units = '0' AND status = 'matured'
            AND NOT EXISTS (SELECT 1 FROM swyp_ledger_entries WHERE ref_type='stake_bonus' AND ref_id=$1)`,
        [s.id],
      );
      // principalul a fost deja returnat — doar bonusul așteaptă bugetul lunii
      // următoare; la următoarea rulare stake-ul e reprocesat doar pt bonus.
    }
  }
