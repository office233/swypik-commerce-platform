...
- **Cloudflare Tunnel `swypik-home`** (systemd `cloudflared`, config /etc/cloudflared/config.yml): swypik.com+www→localhost:3005, cdn→:9000, api→:8090, erp+*.erp→:8091
- Cron-uri (crontab dev): daily-maintenance 04:15, dispatch-tick ~10s, @reboot swypik-stack-up.sh
...
## Resurse
- Backup complet migrare: `E:\vps-migrate\` (dump-uri DB, MinIO, env-uri, nginx original)
