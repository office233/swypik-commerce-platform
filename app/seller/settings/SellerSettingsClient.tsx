...
export default function SellerSettingsClient({ initialData }: Props) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg(data.message || "Setările au fost salvate cu succes!");
      setUsername(data.username);
    } catch (err: any) {
      setErrorMsg(err.message || "Nu s-au putut salva setările.");
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[#E5E5E5] pb-5">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-black text-[#0D0D0D]">Setări Magazin & Profil Public</h1>
            <span className="px-2 py-0.5 rounded-full text-xs font-bold bg-violet-100 text-violet-700">
              Swypik Business
            </span>
          </div>
          <p className="text-sm text-neutral-500 mt-1">
            Configurează identitatea magazinului tău și datele de facturare ale firmei.
          </p>
        </div>
        ...
        {cleanHandle && (
          ...
          >
            <ExternalLink size={16} /> Vezi Magazinul pe Swypik
          </Link>
        )}
      </div>
      ...
      <form onSubmit={handleSave} className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <div className="bg-white border border-[#E5E5E5] rounded-2xl p-6 shadow-sm space-y-5">
            <div className="flex items-center gap-2 text-lg font-bold text-[#0D0D0D]">
              ...
              Profilul Public al Magazinului tău
            </div>
            <p className="text-xs text-neutral-500">
              Cumpărătorii de pe Swypik vor vedea aceste date pe profilul tău de utilizator și la fiecare produs din feed.
            </p>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-neutral-700 uppercase tracking-wider mb-1.5">
                  Nume Magazin (Display Name) *
                </label>
                <input
                  type="text"
                  required
                  ...
                  placeholder="ex: TechGadgets România"
              ...
              <div>
                ...
                <p className="text-[11px] text-neutral-400 mt-1">
                  Link-ul tău: <span className="text-violet-600 font-semibold">swypik.com/u/{cleanHandle || "username"}</span>
                </p>
              </div>
            </div>
            ...
            <div>
              <label className="block text-xs font-bold text-neutral-700 uppercase tracking-wider mb-1.5">
                Descriere Magazin / Bio
              </label>
              <textarea
                rows={3}
                value={bio}
                onChange={(e) => setBio(e.target.value)}
                placeholder="Spune-le cumpărătorilor cine ești și ce produse oferi (ex: Gadgeturi virale testate, livrare în 24h la Easybox)..."
          ...
          <div className="bg-white border border-[#E5E5E5] rounded-2xl p-6 shadow-sm space-y-5">
            <div className="flex items-center gap-2 text-lg font-bold text-[#0D0D0D]">
              ...
              Date Fiscale & Facturare ERP
            </div>
            <p className="text-xs text-neutral-500">
              Aceste date apar pe facturile emise clienților și sunt folosite pentru plata vânzărilor tale.
            </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                />
                <p className="text-[11px] text-neutral-400 mt-1">Ex: FACT-001, SW-2026</p>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white border border-[#E5E5E5] rounded-2xl p-5 shadow-sm space-y-4 text-center">
            ...
            <div>
              <h3 className="font-black text-lg text-[#0D0D0D]">{name || "Numele Magazinului"}</h3>
              <p className="text-xs font-bold text-violet-600">@{cleanHandle || "username"}</p>
            </div>

            <p className="text-xs text-neutral-600 line-clamp-3 italic">
              &ldquo;{bio || "Descrierea magazinului tău va apărea aici pentru toți clienții din aplicație."}&rdquo;
            </p>
            ...
            <div className="pt-3 border-t border-[#E5E5E5] grid grid-cols-2 gap-2 text-xs">
              <div className="bg-neutral-50 p-2 rounded-lg font-medium text-neutral-600">
                <span className="block font-bold text-[#0D0D0D]">Social Feed</span> Activ
              </div>
              <div className="bg-neutral-50 p-2 rounded-lg font-medium text-neutral-600">
                <span className="block font-bold text-[#0D0D0D]">Comision</span> 7%
              </div>
            </div>
