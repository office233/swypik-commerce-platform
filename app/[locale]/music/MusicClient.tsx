 ...
 * de acasă (`MusicHomeRows`) și filtrul pe genuri (chip → listă `TrackRow`).
 * Mirror-uiește `app/[locale]/movies/MoviesClient.tsx`, dar violet (`#7C3AED`)
 * și cu redare directă în loc de navigare pe carduri.
 */
...
export default function MusicClient() {
    ...
    return (
        <main className={`${moviesDisplayFont.variable} min-h-screen bg-gradient-to-b from-[#0B0B12] to-black pb-24 text-white`}>
                ...
                <>
                    {featured ? (
                        <section className="relative h-[62vh] w-full overflow-hidden pt-14">
                            {featured.coverUrl && (
                                <Image src={featured.coverUrl} alt="" fill priority sizes="100vw" className="scale-110 object-cover opacity-60 blur-2xl" />
                            )}
                            ...
                            <div className="absolute inset-x-0 bottom-0 flex items-end gap-4 px-5 pb-6">
                                <div className="relative h-28 w-28 shrink-0 overflow-hidden rounded-xl bg-white/10 shadow-2xl ring-1 ring-white/15 sm:h-32 sm:w-32">
                                    {featured.coverUrl && <Image src={featured.coverUrl} alt={featured.title} fill sizes="128px" className="object-cover" />}
                                </div>
