...
function legacyFilterProducts(products: ProductModel[], query: string, limit = 20, opts: { maxPrice?: number; sort?: string } = {}) { const terms = query.toLowerCase().trim().split(/\s+/).filter(Boolean); let result = rankProducts(products.filter((p) => { const haystack = `${p.title} ${p.description} ${p.category} ${p.vendor} ${p.sku}`.toLowerCase(); const matches = terms.length === 0 || terms.some((term) => haystack.includes(term)); const priceOk = opts.maxPrice == null || p.price <= opts.maxPrice; return matches && priceOk; })); if (opts.sort === "price_asc") result = result.sort((a, b) => a.price - b.price || (b.commerceScore || 0) - (a.commerceScore || 0)); else result = result.sort((a, b) => (b.commerceScore || 0) - (a.commerceScore || 0) || b.qualityScore - a.qualityScore); return result.slice(0, limit); }
function smartRecommend(products: ProductModel[], query: string, limit = 16, opts: { maxPrice?: number; debug?: boolean } = {}) { const result = recommendProducts(products, query, { maxPrice: opts.maxPrice, limit, debug: opts.debug }); const recommended = result.products as ProductModel[]; if (recommended.length >= Math.min(4, limit)) return recommended; const fallback = legacyFilterProducts(products, query, limit, { maxPrice: opts.maxPrice }); const seen = new Set(recommended.map((p) => p.id)); return [...recommended, ...fallback.filter((p) => !seen.has(p.id))].slice(0, limit); }
function uniqueProducts(products: ProductModel[]) { return products.filter((p, idx, arr) => arr.findIndex((x) => x.id === p.id) === idx); }
function buildBundleProducts(mainProducts: ProductModel[], allProducts: ProductModel[], bundleQueries: string[], baseQuery: string, maxPrice?: number) { const ruleBased = recommendComplements(allProducts, mainProducts, baseQuery, { maxPrice, limit: 12 }).products as ProductModel[]; const queryBased = bundleQueries.flatMap((query) => smartRecommend(allProducts, query, 6, { maxPrice })); const engineBased = mainProducts.slice(0, 3).flatMap((product) => pickBundleProducts(product, allProducts, 6)); return uniqueProducts([...ruleBased, ...queryBased, ...engineBased]).filter((p) => !mainProducts.some((main) => main.id === p.id)).filter((p) => maxPrice == null || p.price <= maxPrice).slice(0, 12); }
...
export async function POST(req: Request) {
  try {
    ...
    if (directQuery) {
      const products = smartRecommend(allProducts, directQuery, 20, { maxPrice: baseSession.budget });
      const bundleProducts = buildBundleProducts(products, allProducts, inferBundleQueries(directQuery), directQuery, baseSession.priceSensitivity === "high" ? baseSession.budget : undefined);
      const suggestion = products[0] ? ` ${buildSalesSuggestion(products[0], bundleProducts.slice(0, 2))}` : "";
      return NextResponse.json({ intent: "search_product", reply: products.length ? `Am găsit ${products.length} produse potrivite. Alege unul și îți fac bundle instant. 🔥${suggestion}` : "Nu am găsit produse relevante în Shopify pentru căutarea asta. Încearcă o categorie mai clară sau verifică tags/descrieri în Shopify.", products, bundleProducts, shoppingSession: baseSession, sessionId: sessionId || crypto.randomUUID() });
    }
    ...
    if (aiResult.intent === "search_product" || aiResult.intent === "find_cheaper") {
      ...
      const products = smartRecommend(allProducts, query, 16, { maxPrice });
      const bundleQueries = [...(aiResult.bundleQueries || []), ...inferBundleQueries(query)];
      const bundleProducts = buildBundleProducts(products, allProducts, bundleQueries, query, maxPrice);
