...
export default function ChatInterface() {
  ...
  useEffect(() => { setSessionId(Math.random().toString(36).slice(2) + Date.now().toString(36)); fetch("/api/products?mode=trending&limit=20").then((r) => r.json()).then((d) => setTrendingProducts(d.products || [])).catch(() => {}); fetch("/api/products?hierarchy=true").then(r=>r.json()).then(d=>setCategoryTree(d.hierarchy||[])).catch(()=>{}); }, []);
  ...
  const ProductCard = ({ product, compact = false }: { product: ChatProduct; compact?: boolean }) => { const badge = product.commerceBadge || product.socialProofLabel; return <div className={`${compact ? "w-48 shrink-0" : ""} overflow-hidden rounded-2xl bg-white border border-[#E5E5E5] hover:border-[#D1D1D6] hover:shadow-md transition-all cursor-pointer`} onClick={() => setSelectedProduct(product)}><div className="relative h-40 bg-[#F7F7F8]">{product.images?.[0] ? <img src={product.images[0]} alt={product.title} className="h-full w-full object-cover" /> : <div className="grid h-full place-items-center"><Package className="text-[#D1D1D6]" /></div>}{product.discountPercent > 0 && <span className="absolute right-2 top-2 rounded-full bg-[#EF4444] px-2.5 py-1 text-[10px] font-black text-white">-{product.discountPercent}%</span>}{badge && <span className={`absolute left-2 top-2 max-w-[80%] rounded-full px-2.5 py-1 text-[10px] font-black shadow ${commerceBadgeClass(badge)}`}>{badge}</span>}</div><div className="p-3"><p className="line-clamp-2 text-sm font-bold leading-tight text-[#0D0D0D]">{product.title}</p><div className="mt-1 flex items-center gap-2 text-[11px] font-medium text-[#6E6E80]"><span className="text-[#F59E0B]">★ {product.rating?.toFixed?.(1) || "4.8"}</span><span>{product.orders || 0}+ comenzi</span></div><div className="mt-2 flex items-end gap-2"><span className="text-xl font-black text-[#10A37F]">{product.price} lei</span>{product.oldPrice > product.price && <span className="text-xs text-[#6E6E80] line-through">{product.oldPrice} lei</span>}</div><button onClick={(e) => { e.stopPropagation(); addToCart(product); }} className={`mt-3 w-full rounded-xl py-2.5 text-sm font-bold ${THEME.classes.cartButton}`}>Adaugă în coș</button></div></div>; };
  const ProductSkeleton = ({ compact = false }) => <div className={`${compact ? "w-48 shrink-0" : ""} overflow-hidden rounded-2xl bg-white border border-[#E5E5E5] animate-pulse`}><div className="h-40 bg-[#F7F7F8]"></div><div className="p-3"><div className="h-4 bg-[#F7F7F8] rounded mb-2 w-3/4"></div><div className="h-4 bg-[#F7F7F8] rounded mb-4 w-1/2"></div><div className="h-6 bg-[#F7F7F8] rounded w-1/3"></div><div className="mt-3 h-10 w-full rounded-xl bg-[#F7F7F8]"></div></div></div>;
  ...
  return <main className={`min-h-screen ${THEME.classes.appBg}`} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}><div className={`relative mx-auto min-h-screen max-w-lg ${THEME.classes.pageBg}`}>{activeTab !== "feed" && <header className="sticky top-0 z-30 border-b border-[#E5E5E5] bg-white/95 px-4 py-3 backdrop-blur-xl"><div className="flex items-center justify-between"><button onClick={() => setActiveTab("home")} className="flex items-center gap-2"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[#0D0D0D] text-sm font-black text-white">AI</span><div className="text-left"><h1 className="text-lg font-black text-[#0D0D0D]">AICeVrei.ro</h1><p className="text-[10px] font-semibold text-[#6E6E80]">{funnelStage === "compare" ? "compară și alege rapid" : funnelStage === "upsell" ? "completează bundle-ul" : "Shopping inteligent cu AI"}</p></div></button><button onClick={() => setActiveTab("cart")} className="relative rounded-xl bg-[#0D0D0D] p-2.5 text-white"><ShoppingCart size={18} />{cartCount > 0 && <span className="absolute -right-1 -top-1 grid h-5 w-5 place-items-center rounded-full bg-[#10A37F] text-[10px] font-black text-white">{cartCount}</span>}</button></div></header>}
    <section className={activeTab === "feed" ? "h-screen" : "min-h-[calc(100vh-132px)] pb-36"}>{activeTab === "home" && <div className="px-4 pt-6"><div className="rounded-2xl border border-[#E5E5E5] bg-[#F7F7F8] p-6"><h2 className="text-3xl font-black leading-tight tracking-tight text-[#0D0D0D]">Caută orice produs.<br/>AI-ul îți face și bundle.</h2><p className="mt-2 text-sm font-medium text-[#6E6E80]">Search instant în 108.000+ produse. Zero limite.</p><div className="relative"><div className={`mt-4 flex items-center gap-2 rounded-xl p-3 ${THEME.classes.softInput}`}><Search size={20} className="text-[#6E6E80]" /><input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") runRealSearch(input); }} className="w-full bg-transparent text-sm font-medium text-[#0D0D0D] outline-none placeholder:text-[#A1A1AA]" placeholder="Ex: rochie, cadou, sub 100 lei..." /></div>{suggestions.length > 0 && <div className="absolute left-0 right-0 top-full z-40 mt-1 overflow-hidden rounded-xl bg-white shadow-lg border border-[#E5E5E5]">{suggestions.map((s) => <button key={`${s.type}-${s.label}`} onClick={() => runRealSearch(s.label)} className="flex w-full items-center justify-between px-4 py-3 text-left text-sm font-medium text-[#0D0D0D] hover:bg-[#F7F7F8]"><span>{s.label}</span><span className="rounded-full bg-[#F7F7F8] px-2 py-0.5 text-[10px] font-bold text-[#6E6E80]">{s.type}</span></button>)}</div>}</div><div className="mt-3 grid grid-cols-2 gap-2"><button onClick={() => runRealSearch(input)} disabled={!input.trim() || searchLoading} className={`rounded-xl py-3.5 font-bold disabled:opacity-50 ${THEME.classes.primaryButton}`}>Caută</button><button onClick={() => sendMessage(input)} disabled={!input.trim() || isLoading} className="rounded-xl bg-[#0D0D0D] py-3.5 font-bold text-white disabled:opacity-50">Întreabă AI</button></div></div>
    <div className="mt-4"><button onClick={() => setActiveTab("categories")} className="w-full rounded-xl bg-[#0D0D0D] py-3 font-bold text-white flex items-center justify-center gap-2"><Grid3x3 size={16} /> Explorează categorii</button></div>
    {searchLoading ? <ProductCarousel title={`Se caută rezultate...`} isLoading={true} /> : searchResults.length > 0 && <ProductCarousel title={`Rezultate (${searchResults.length})`} products={searchResults} />}<ProductCarousel title="Trending acum" products={trendingProducts.slice(0, 10)} /></div>}
    {activeTab === "categories" && <div className="px-4 pt-4">{catBrowsing ? <div><div className="flex items-center gap-3 mb-4"><button onClick={() => { setCatBrowsing(false); setCatProducts([]); setCatTitle(""); }} className="grid h-9 w-9 place-items-center rounded-xl bg-[#F7F7F8] border border-[#E5E5E5]"><ChevronDown size={16} className="rotate-90" /></button><div className="flex-1"><p className="text-lg font-black text-[#0D0D0D]">{catTitle.split(" > ").map(p => translateCategory(p.trim())).join(" → ")}</p><p className="text-xs text-[#6E6E80]">{catTotal} produse</p></div><button onClick={() => setShowFilters(!showFilters)} className={`flex items-center gap-1 rounded-xl px-3 py-2 text-xs font-bold border transition-all ${showFilters ? "bg-[#0D0D0D] text-white border-[#0D0D0D]" : "bg-white text-[#0D0D0D] border-[#E5E5E5]"}`}><SlidersHorizontal size={14} /></button></div>{showFilters && <div className="mb-4 rounded-xl bg-[#F7F7F8] p-4 border border-[#E5E5E5] animate-slideDown space-y-3"><div><label className="text-xs font-bold text-[#6E6E80] uppercase">Sortare</label><div className="mt-1 flex flex-wrap gap-1.5">{[{v:"popular",l:"Popular"},{v:"price_asc",l:"Preț ↑"},{v:"price_desc",l:"Preț ↓"},{v:"newest",l:"Noi"},{v:"discount",l:"Reduceri"}].map(s=><button key={s.v} onClick={()=>{setFilterSort(s.v); browseCategory(catTitle, true, 1);}} className={`sub-chip rounded-lg px-3 py-1.5 text-xs font-semibold ${filterSort===s.v?"active":""}`}>{s.l}</button>)}</div></div><div><label className="text-xs font-bold text-[#6E6E80] uppercase">Preț maxim: {filterMaxPrice} lei</label><input type="range" min={20} max={2000} step={10} value={filterMaxPrice} onChange={e=>{setFilterMaxPrice(Number(e.target.value));}} onMouseUp={()=>browseCategory(catTitle,true,1)} onTouchEnd={()=>browseCategory(catTitle,true,1)} className="mt-1 w-full accent-[#10A37F]" /></div></div>}<div className="grid grid-cols-2 gap-3">{catProducts.map(p => <ProductCard key={p.id} product={p} />)}{catLoading && <><ProductSkeleton /><ProductSkeleton /><ProductSkeleton /><ProductSkeleton /></>}</div>{!catLoading && catProducts.length < catTotal && <button onClick={() => browseCategory(catTitle, true, catPage + 1)} className="mt-5 w-full rounded-xl bg-[#0D0D0D] py-4 text-sm font-bold text-white shadow-lg">Mai afișează +20 produse (din {catTotal})</button>}</div> : <><div className="flex items-center justify-between mb-4"><h2 className="text-2xl font-black text-[#0D0D0D]">Categorii</h2></div><div className="space-y-2">{categoryTree.map((cat:any)=><div key={cat.name} className="rounded-xl border border-[#E5E5E5] overflow-hidden"><button onClick={()=>{setExpandedCat(expandedCat===cat.name?null:cat.name);setExpandedMid(null);}} className={`w-full flex items-center justify-between p-4 text-left font-bold transition-colors ${expandedCat===cat.name?"bg-[#0D0D0D] text-white":"bg-white text-[#0D0D0D] hover:bg-[#F7F7F8]"}`}><span className="text-sm">{translateCategory(cat.name)}</span><div className="flex items-center gap-2"><span className="text-[10px] opacity-60">{cat.count.toLocaleString()}</span><ChevronDown size={16} className={`transition-transform ${expandedCat===cat.name?"rotate-180":""}`}/></div></button>{expandedCat===cat.name&&<div className="p-3 bg-[#FAFAFA] border-t border-[#E5E5E5] animate-slideDown"><div className="grid grid-cols-2 gap-2">{cat.children.map((mid:any)=><button key={mid.name} onClick={()=>{setExpandedMid(expandedMid===mid.name?null:mid.name);}} className={`rounded-lg p-3 text-left border transition-all ${expandedMid===mid.name?"bg-[#10A37F] text-white border-[#10A37F]":"bg-white border-[#E5E5E5] hover:border-[#10A37F]"}`}><span className="text-xs font-bold block">{translateCategory(mid.name)}</span><span className="text-[10px] opacity-70">{mid.count.toLocaleString()} produse</span></button>)}</div>{expandedMid&&<div className="mt-3 flex flex-wrap gap-1.5 animate-slideDown">{cat.children.find((m:any)=>m.name===expandedMid)?.children.map((sub:any)=><button key={sub.name} onClick={()=>browseCategory(`${cat.name} > ${expandedMid} > ${sub.name}`,true)} className={`sub-chip rounded-lg px-3 py-2 text-xs font-semibold ${activeSub===`${cat.name} > ${expandedMid} > ${sub.name}`?"active":""}`}>{translateCategory(sub.name)} <span className="opacity-60">({sub.count.toLocaleString()})</span></button>)}</div>}</div>}</div>)}</div></>}</div>}
    {activeTab === "chat" && <div className="px-4 pt-4"><div className="space-y-4">{messages.map((m) => <div key={m.id} className={m.role === "user" ? "text-right" : "text-left"}><div className={`inline-block max-w-[88%] rounded-2xl px-4 py-3 text-sm font-medium ${m.role === "user" ? "bg-[#0D0D0D] text-white" : "bg-[#F7F7F8] text-[#0D0D0D] border border-[#E5E5E5]"}`}>{m.role === "assistant" && <div className="mb-1 flex items-center gap-1 text-xs font-bold text-[#10A37F]"><Bot size={13} /> AI Seller</div>}<p className="whitespace-pre-wrap">{m.content}</p></div>{m.role === "assistant" && <><ProductCarousel title="Recomandate pentru tine" products={m.products} /><ProductCarousel title="Completează bundle-ul" products={m.bundleProducts} /></>}</div>)}{isLoading && <div className="rounded-xl bg-[#F7F7F8] p-3 text-sm font-medium text-[#6E6E80] border border-[#E5E5E5]">A     {activeTab === "deals" && <div className="px-4 pt-4"><h2 className="mb-3 text-2xl font-black text-[#0D0D0D]">Reduceri</h2>{dealsLoading ? <p className="py-20 text-center font-medium text-[#6E6E80]">Se încarcă...</p> : <div className="grid grid-cols-2 gap-3">{dealsProducts.map((p) => <ProductCard key={p.id} product={p} />)}</div>}</div>}{activeTab === "feed" && <ProductFeed products={feedProducts} onAddToCart={addToCart} onLoadMore={loadMoreFeed} onClose={() => setActiveTab("home")} isLoading={feedLoading} />}{activeTab === "cart" && <div className="px-4 pt-4 pb-10"><h2 className="mb-4 text-2xl font-black text-[#0D0D0D]">Coșul tău</h2>{cartItems.length === 0 ? <p className="py-20 text-center font-medium text-[#6E6E80]">Coșul este gol.</p> : <><div className="space-y-3">{cartItems.map((item, i) => <div key={item.product.id} className="flex gap-3 rounded-2xl bg-[#F7F7F8] p-3 border border-[#E5E5E5]">{item.product.images?.[0] && <img src={item.product.images[0]} alt="" className="h-16 w-16 rounded-xl object-cover" />}<div className="flex-1"><p className="line-clamp-2 text-sm font-bold text-[#0D0D0D]">{item.product.title}</p><p className="text-xs font-bold text-[#10A37F]">{item.product.price * item.qty} lei</p></div><div className="flex items-center gap-2"><button onClick={() => updateQty(i, -1)} className="grid h-8 w-8 place-items-center rounded-full bg-[#E5E5E5] font-bold text-[#0D0D0D] hover:bg-[#D1D1D6] active:scale-90 transition-transform">-</button><span className="w-4 text-center font-bold">{item.qty}</span><button onClick={() => updateQty(i, 1)} className="grid h-8 w-8 place-items-center rounded-full bg-[#E5E5E5] font-bold text-[#0D0D0D] hover:bg-[#D1D1D6] active:scale-90 transition-transform">+</button></div></div>)}</div><div className="mt-8 border-t border-[#E5E5E5] pt-6"><ProductCarousel title="🔥 Adaugă la ofertă și:" products={trendingProducts.slice(0, 5)} /></div><div className="mt-5 rounded-2xl bg-[#F7F7F8] p-4 border border-[#E5E5E5]"><div className="flex justify-between text-xl font-black"><span>Total</span><span className="text-[#10A37F]">{cartTotal} lei</span></div><button onClick={submitOrder} disabled={checkoutLoading} className={`mt-4 w-full rounded-xl py-4 font-bold disabled:opacity-50 ${THEME.classes.cartButton} active:scale-[0.98] transition-transform`}>{checkoutLoading ? "Se procesează..." : `Finalizează comanda — ${cartTotal} lei`}</button><button onClick={() => setShowBundleSheet(true)} className="mt-2 w-full rounded-xl bg-[#10A37F] py-3 font-bold text-white hidden">Vezi bundle complet</button></div></>}</div>}</section> py-3 font-bold text-white">Vezi bundle complet</button></div></>}</div>}</section>
    {activeTab !== "feed" && <div className="fixed bottom-14 left-1/2 z-30 w-full max-w-lg -translate-x-1/2 border-t border-[#E5E5E5] bg-white/95 px-3 py-2 backdrop-blur-xl"><div className={`flex gap-2 rounded-xl p-2 ${THEME.classes.softInput}`}><input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && sendMessage()} className="flex-1 bg-transparent px-2 text-sm font-medium text-[#0D0D0D] outline-none placeholder:text-[#A1A1AA]" placeholder="Scrie ce cauți..." /><button onClick={() => sendMessage()} disabled={!input.trim() || isLoading} className="grid h-10 w-10 place-items-center rounded-xl bg-[#0D0D0D] text-white disabled:opacity-40"><Send size={16} /></button></div></div>}{activeTab !== "feed" && <nav className="fixed bottom-0 left-1/2 z-40 w-full max-w-lg -translate-x-1/2 border-t border-[#E5E5E5] bg-white/95 px-4 py-2 backdrop-blur-xl"><div className="flex justify-around text-[10px] font-bold text-[#6E6E80]"><NavBtn icon={<Home size={18} />} label="Acasă" active={activeTab === "home"} onClick={() => setActiveTab("home")} /><NavBtn icon={<Grid3x3 size={18} />} label="Categorii" active={activeTab === "categories"} onClick={() => { setActiveTab("categories"); setCatBrowsing(false); }} /><NavBtn icon={<Flame size={18} />} label="Feed" active={activeTab === "feed"} onClick={() => setActiveTab("feed")} /><NavBtn icon={<MessageCircle size={18} />} label="Chat" active={activeTab === "chat"} onClick={() => setActiveTab("chat")} /><NavBtn icon={<ShoppingCart size={18} />} label={`Coș ${cartCount ? `(${cartCount})` : ""}`} active={activeTab === "cart"} onClick={() => setActiveTab("cart")} /></div></nav>}
...
function NavBtn({ icon, label, active, onClick }: { icon: React.ReactNode; label: string; active: boolean; onClick: () => void }) { return <button onClick={onClick} className={`flex flex-col items-center gap-0.5 transition-colors ${active ? "text-[#0D0D0D]" : "text-[#A1A1AA]"}`}>{icon}{label}</button>; }
function ProductModal({ product, onClose, onAddToCart }: { product: ChatProduct; onClose: () => void; onAddToCart: () => void }) { return <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/30" onClick={onClose}><div className="max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-t-[2rem] bg-white p-5" onClick={(e) => e.stopPropagation()}><div className="mb-3 flex justify-between"><span className="rounded-full bg-[#F7F7F8] px-3 py-1 text-xs font-bold text-[#6E6E80]">{product.category}</span><button onClick={onClose}><X size={18} /></button></div>{product.images?.[0] && <img src={product.images[0]} alt={product.title} className="h-64 w-full rounded-2xl object-cover" />}<h2 className="mt-4 text-2xl font-black text-[#0D0D0D]">{product.title}</h2><div className="mt-2 flex gap-3 text-sm font-medium text-[#6E6E80]"><span className="text-[#F59E0B]"><Star size={14} className="inline" fill="currentColor" /> {product.rating}</span><span>{product.orders}+ comenzi</span><span><Truck size={14} className="inline" /> {product.deliveryDays} zile</span></div><div className="mt-3"><span className="text-3xl font-black text-[#10A37F]">{product.price} lei</span>{product.oldPrice > product.price && <span className="ml-2 text-[#6E6E80] line-through">{product.oldPrice} lei</span>}</div><p className="mt-4 text-sm font-medium leading-relaxed text-[#6E6E80]">{product.description}</p>{product.benefits?.length > 0 && <div className="mt-4 space-y-2">{product.benefits.map((b, i) => <div key={i} className="rounded-xl bg-[#F7F7F8] px-3 py-2 text-sm font-medium text-[#0D0D0D]">✓ {b}</div>)}</div>}<button onClick={onAddToCart} className={`mt-5 w-full rounded-xl py-4 font-bold ${THEME.classes.cartButton}`}>Adaugă în coș — {product.price} lei</button></div></div>; }
