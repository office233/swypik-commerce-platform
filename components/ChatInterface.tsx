...
export default function ChatInterface({ 
  ...
  return <main className={`min-h-screen ${THEME.classes.appBg}`}><div className={`relative mx-auto min-h-screen max-w-lg md:max-w-2xl lg:max-w-4xl xl:max-w-6xl app-container ${THEME.classes.pageBg}`}>{activeTab !== "feed" && <header className="sticky top-0 z-30 border-b border-[#E5E5E5] bg-white/95 px-4 py-3 backdrop-blur-xl safe-top"><div className="flex items-center justify-between"><button type="button" onClick={() => setActiveTab("home")} className="flex items-center gap-2"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[#0D0D0D] text-sm font-black text-white">AI</span><div className="text-left"><h1 className="text-lg font-black text-[#0D0D0D]">AICeVrei.ro</h1><p className="text-[10px] font-semibold text-[#6E6E80]">{funnelStage === "compare" ? "compară și alege rapid" : funnelStage === "upsell" ? "completează bundle-ul" : "Asistentul tău de shopping"}</p></div></button><button type="button" onClick={() => setActiveTab("cart")} className="relative rounded-xl bg-[#0D0D0D] p-2.5 text-white"><ShoppingCart size={18} />{cartCount > 0 && <span className="absolute -right-1 -top-1 grid h-5 w-5 place-items-center rounded-full bg-[#10A37F] text-[10px] font-black text-white">{cartCount}</span>}</button></div></header>}
      ...
      {activeTab === "home" && (
        <div className="px-4 pt-6">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {QUICK_PROMPTS.map((qa) => <button type="button" key={qa.label} onClick={() => sendMessage(qa.query)} className="rounded-full bg-white border border-[#E5E5E5] px-3.5 py-2 text-xs font-bold text-[#0D0D0D] hover:border-[#10A37F] hover:text-[#10A37F] active:scale-95 transition-all">{qa.label}</button>)}
          </div>
          <div className="mt-4">
