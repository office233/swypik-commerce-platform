...
export default function ChatInterface({ 
  ...
  const [messages, setMessages] = useState<ChatMessage[]>(() => { try { const saved = typeof window !== "undefined" && localStorage.getItem("aicv_chat"); if (saved) { const parsed = JSON.parse(saved); return parsed.map((m: any) => ({ ...m, timestamp: new Date(m.timestamp) })); } } catch {} return []; });
  ...
  const [cartItems, setCartItems] = useState<CartItem[]>(() => { try { const saved = typeof window !== "undefined" && localStorage.getItem("aicv_cart"); if (saved) return JSON.parse(saved); } catch {} return []; });
  ...
  const ProductCard = ({ product, compact = false }: { product: ChatProduct; compact?: boolean }) => {
    ...
    return (
      <div className={`${compact ? "w-[10.5rem] sm:w-[11.5rem] shrink-0 carousel-card" : ""} overflow-hidden rounded-2xl bg-white border border-[#E5E5E5] md:hover:border-[#D1D1D6] md:hover:shadow-md transition-all cursor-pointer active:scale-[0.98]`} onClick={() => router.push(`/product/${product.pgId || product.id}`)}>
        <div className="relative h-40 sm:h-44 bg-[#F7F7F8] product-card-image">
        ...
        <div className="p-3">
          ...
          <div className="mt-2 flex items-end gap-2">
            <span className="text-base sm:text-lg font-black text-[#10A37F] product-card-price">{product.price} lei</span>
          ...
          <div className="mt-2 flex items-center gap-1.5">
            <div className="flex items-center rounded-lg border border-[#E5E5E5] overflow-hidden" onClick={e => e.stopPropagation()}>
              <button onClick={(e) => { e.stopPropagation(); updateCardQty(product.id, -1); }} className="qty-btn grid h-8 w-8 place-items-center text-[#6E6E80] hover:bg-[#F7F7F8] active:scale-90 transition-all text-sm font-bold">−</button>
  ...
  return <main className={`min-h-screen ${THEME.classes.appBg}`}><div className={`relative mx-auto min-h-screen max-w-lg md:max-w-2xl lg:max-w-4xl xl:max-w-6xl app-container ${THEME.classes.pageBg}`}>{activeTab !== "feed" && <header className="sticky top-0 z-30 border-b border-[#E5E5E5] bg-white/95 px-4 py-3 backdrop-blur-xl safe-top"><div className="flex items-center justify-between"><button onClick={() => setActiveTab("home")} className="flex items-center gap-2"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[#0D0D0D] text-sm font-black text-white">AI</span><div className="text-left"><h1 className="text-lg font-black text-[#0D0D0D]">AICeVrei.ro</h1><p className="text-[10px] font-semibold text-[#6E6E80]">{funnelStage === "compare" ? "compară și alege rapid" : funnelStage === "upsell" ? "completează bundle-ul" : "Asistentul tău de shopping"}</p></div></button><button onClick={() => setActiveTab("cart")} className="relative rounded-xl bg-[#0D0D0D] p-2.5 text-white"><ShoppingCart size={18} />{cartCount > 0 && <span className="absolute -right-1 -top-1 grid h-5 w-5 place-items-center rounded-full bg-[#10A37F] text-[10px] font-black text-white">{cartCount}</span>}</button></div></header>}
    ...
    {activeTab === "chat" && <div className="fixed z-30 w-full max-w-lg md:max-w-2xl lg:max-w-4xl xl:max-w-6xl left-1/2 -translate-x-1/2 border-t border-[#E5E5E5] bg-white/95 px-3 py-2 backdrop-blur-xl chat-input-bar" onTouchStart={e=>e.stopPropagation()} onTouchEnd={e=>e.stopPropagation()}><div className={`flex gap-2 rounded-xl p-2 ${THEME.classes.softInput}`}><input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && sendMessage()} className="flex-1 bg-transparent px-2 text-base font-medium text-[#0D0D0D] outline-none placeholder:text-[#A1A1AA]" placeholder="Scrie ce cauți..." /><button onClick={() => sendMessage()} disabled={!input.trim() || isLoading} className="grid h-10 w-10 place-items-center rounded-xl bg-[#0D0D0D] text-white disabled:opacity-40"><Send size={16} /></button></div></div>}{(activeTab as string) !== "feed" && <nav className="fixed bottom-0 left-1/2 z-40 w-full max-w-lg md:max-w-2xl lg:max-w-4xl xl:max-w-6xl -translate-x-1/2 border-t border-[#E5E5E5] bg-white/95 px-4 py-2 backdrop-blur-xl bottom-nav"><div className="flex justify-around text-[10px] font-bold text-[#6E6E80]"><NavBtn icon={<Home size={20} />} label="Acasă" active={activeTab === "home"} onClick={() => setActiveTab("home")} /><NavBtn icon={<Grid3x3 size={20} />} label="Categorii" active={activeTab === "categories"} onClick={() => { setActiveTab("categories"); setCatBrowsing(false); }} /><NavBtn icon={<Flame size={20} />} label="Feed" active={(activeTab as string) === "feed"} onClick={() => setActiveTab("feed")} /><NavBtn icon={<MessageCircle size={20} />} label="Chat" active={activeTab === "chat"} onClick={() => setActiveTab("chat")} /><NavBtn icon={<ShoppingCart size={20} />} label={`Coș ${cartCount ? `(${cartCount})` : ""}`} active={activeTab === "cart"} onClick={() => setActiveTab("cart")} /></div></nav>}
