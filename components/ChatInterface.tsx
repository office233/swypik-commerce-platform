...
export default function ChatInterface() {
  ...
  useEffect(() => { setSessionId(Math.random().toString(36).slice(2) + Date.now().toString(36)); fetch("/api/products?mode=trending&limit=12").then((r) => r.json()).then((d) => setTrendingProducts(d.products || [])).catch(() => {}); fetch("/api/products?mode=bestvalue&limit=12").then((r) => r.json()).then((d) => setBestValueProducts(d.products || [])).catch(() => {}); fetch("/api/products?mode=toprated&limit=12").then((r) => r.json()).then((d) => setTopRatedProducts(d.products || [])).catch(() => {}); fetch("/api/products?hierarchy=true").then(r=>r.json()).then(d=>setCategoryTree(d.hierarchy||[])).catch(()=>{}); }, []);
    ...
    <section className={activeTab === "feed" ? "h-screen" : "min-h-[calc(100vh-132px)] pb-36"} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
      {activeTab === "home" && (
        <div className="px-4 pt-6">
          ...
          <ProductCarousel title="🔥 Popular acum" products={trendingProducts.slice(0, 10)} />
          <ProductCarousel title="🏆 Best Value — AI Pick" products={bestValueProducts.slice(0, 10)} />
          <ProductCarousel title="⭐ Top Rated (4.7+)" products={topRatedProducts.slice(0, 10)} />
        </div>
      )}
