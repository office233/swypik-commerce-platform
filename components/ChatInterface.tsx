...
import { BookOpen, Bot, Bookmark, ChevronDown, ChevronRight, ClipboardList, Compass, Flame, Grid3x3, Home, LogOut, Menu, MessageCircle, Package, Plus, Search, Send, Shield, ShoppingCart, SlidersHorizontal, Sparkles, Star, Tag, Trophy, Truck, Upload, User, X, Zap } from "lucide-react";
...
// Blog: lazy-loaded so the home tab bundle stays lean.
const BlogHub = dynamicImport(() => import("./blog/BlogHub"), { ssr: false });
const BlogTeaser = dynamicImport(() => import("./blog/BlogTeaser"), { ssr: false });
...
type Tab = "home" | "chat" | "deals" | "feed" | "cart" | "categories" | "blog";
...
const getQuickPrompts = (t: (k: string) => string) => [
  { label: t("qpGamingLabelDefault"), query: t("qpGamingQueryDefault") },
  { label: t("qpNuntaLabelDefault"), query: t("qpNuntaQueryDefault") },
  { label: t("qpApartLabelDefault"), query: t("qpApartQueryDefault") },
  { label: t("qpCadouLabelDefault"), query: t("qpCadouQueryDefault") },
  { label: t("qpBirouLabelDefault"), query: t("qpBirouQueryDefault") },
  { label: t("qpSkincareLabelDefault"), query: t("qpSkincareQueryDefault") },
  { label: t("qpFitnessLabelDefault"), query: t("qpFitnessQueryDefault") },
  { label: t("qpIphoneLabelDefault"), query: t("qpIphoneQueryDefault") },
  { label: t("qpPetLabelDefault"), query: t("qpPetQueryDefault") },
];

const getAiWelcome = (t: (k: string) => string): ChatMessage => ({
  id: "welcome", role: "assistant", timestamp: new Date(),
  content: t("aiWelcomeContent"),
});
...
type FeedApiPayload = { items?: unknown[]; products?: unknown[] };
// Heterogeneous feed item — fields come from either /api/v1/feed bridge or the legacy products list.
// We cast through a loose record so the existing untyped field access below keeps compiling.
type LooseFeedItem = Record<string, any>;

function normalizeSocialFeedProducts(data: FeedApiPayload): ChatProduct[] {
  const rawItems = Array.isArray(data?.items) ? data.items : Array.isArray(data?.products) ? data.products : [];

  return (rawItems as LooseFeedItem[])
    .map((item) => {
      const source: LooseFeedItem = item?.product || item;
...
export default function ChatInterface({ 
  ...
  useEffect(() => {
    try { 
      ...
      if (saved) { 
        ...
        setMessages(parsed.map((m: ChatMessage & { timestamp: string | Date }) => ({ ...m, timestamp: new Date(m.timestamp) }))); 
      } 
    } catch {}
  }, []);
  ...
  const funnelCta = funnelStage === "compare" ? t("funnelVeziAlternative") : funnelStage === "cart" || funnelStage === "upsell" ? t("funnelCompleteazaBundle") : t("cauta");
  ...
  function findProductForAI(data: { products?: ChatProduct[]; bundleProducts?: ChatProduct[]; productId?: string; productTitle?: string }) { const all = [...lastShownProducts, ...(data.products || []), ...(data.bundleProducts || [])]; if (data.productId) return all.find((p) => p.id === data.productId); if (data.productTitle) { const needle = String(data.productTitle).toLowerCase(); return all.find((p) => p.title.toLowerCase().includes(needle)); } return all.length === 1 ? all[0] : null; }
  ...
  async function sendMessage(text?: string) {
    ...
    try {
      ...
      const data = await res.json(); if (!res.ok) throw new Error(data?.error || t("errNuAmPutut"));
      ...
      setMessages((prev) => [...prev, { id: crypto.randomUUID(), role: "assistant", content: data.reply || t("errAmCautat"), products, bundleProducts, timestamp: new Date() }]);
      if (data.sessionId) setSessionId(data.sessionId);
    } catch (err) { setMessages((prev) => [...prev, { id: crypto.randomUUID(), role: "assistant", content: (err as Error)?.message || t("errGenericaAparut"), timestamp: new Date() }]); } finally { setIsLoading(false); }
  }
  ...
  const ProductCard = ({ product, compact = false }: { product: ChatProduct; compact?: boolean }) => {
    ...
    const insight = product.rating >= 4.8 && product.orders >= 200 ? t('insightCalitatePesteMedie') : product.orders >= 500 ? t('insightSellerVerificat') : product.discountPercent >= 25 ? t('insightReducereReala') : product.qualityScore >= 9 ? t('insightBestValue') : null;
      ...
      <div className={`${compact ? "w-[10.5rem] sm:w-[11.5rem] shrink-0 carousel-card" : ""} overflow-hidden rounded-2xl bg-white border border-[#E5E5E5] md:hover:border-[#D1D1D6] md:hover:shadow-md transition-all`}>
        ...
        <div className="p-3">
          ...
          <div className="mt-1 flex items-center gap-2 text-[11px] font-medium text-[#6E6E80]">
            ...
            <span>{product.orders || 0}+ {t("comenziSuffix")}</span>
          </div>
          ...
          <div className="mt-2 flex items-center gap-1.5">
            ...
            <button type="button" onClick={(e) => { e.stopPropagation(); addToCart(product, q); setCardQty(prev => ({ ...prev, [product.id]: 1 })); }} className={`flex-1 rounded-lg py-3 text-xs font-bold min-h-[44px] ${THEME.classes.cartButton}`} style={{touchAction:"manipulation"}}>
              <ShoppingCart size={13} className="mr-1 inline" />{q > 1 ? `${q}x ${t("cos")}` : t("cos")}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="mt-4 overflow-hidden rounded-2xl bg-black relative h-[500px]">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 z-20">
          <div className="flex items-center gap-2 mb-2">
            <span className="rounded-full bg-[#DC2626] px-2 py-1 text-[10px] font-black text-white uppercase tracking-wider animate-pulse">{t("trendingTodayBadge")}</span>
          </div>
  ...
  return <main className={`min-h-screen ${THEME.classes.appBg}`}><div className={`relative mx-auto min-h-screen w-full md:max-w-2xl lg:max-w-4xl xl:max-w-6xl app-container ${THEME.classes.pageBg}`}>{activeTab !== "feed" && <header className="sticky top-0 z-30 border-b border-[#E5E5E5] bg-white/95 px-4 py-3 backdrop-blur-xl safe-top"><div className="flex items-center justify-between h-12"><button type="button" onClick={() => { setActiveTab("home"); setCatBrowsing(false); setExpandedCat(null); if (typeof window !== "undefined") window.scrollTo({ top: 0, behavior: "smooth" }); }} className="flex items-center gap-2" aria-label={t("inapoiAcasa")}><span className="text-2xl font-black text-[#0D0D0D] tracking-tight">Swypik</span></button><div className="flex items-center gap-2"><a href="/cart" aria-label={t("cosulMeu")} className="relative grid h-11 w-11 place-items-center rounded-full bg-[#0D0D0D] text-white active:scale-95 transition-transform focus-visible:ring-2 focus-visible:ring-violet-500 focus-visible:outline-none"><ShoppingCart size={18} />{cartCount > 0 && <span className="absolute top-0.5 right-0.5 grid h-[15px] min-w-[15px] place-items-center rounded-full bg-[#7C3AED] px-1 text-[9px] font-black text-white leading-none ring-2 ring-[#0D0D0D]">{cartCount > 99 ? "99+" : cartCount}</span>}</a></div></div></header>}
    ...
    {showMenu && <div className="fixed inset-0 z-50" role="dialog" aria-modal="true" aria-label="Meniu navigare" onClick={() => setShowMenu(false)}>
      ...
      <div className="absolute right-0 top-0 h-full w-72 bg-white shadow-2xl" onClick={e => e.stopPropagation()} style={{animation: 'slideInRight 0.2s ease-out'}}>
        ...
        <div className="p-3 overflow-y-auto" style={{maxHeight: 'calc(100vh - 120px)'}}>
          ...
          <Link href="/earn" className="flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-bold text-[#0D0D0D] hover:bg-[#F7F7F8] transition">
            <Sparkles size={18} className="text-[#FBBF24]" /> Mine $SWYP
          </Link>
          ...
          <p className="px-4 pt-2 pb-1 text-[10px] font-bold uppercase tracking-widest text-[#A1A1AA]">{t("contSectionLabel")}</p>
      ...
      {activeTab === "home" && (
        <div className="px-4 pt-6">
          <div className="rounded-2xl border border-[#E5E5E5] bg-[#F7F7F8] p-6">
            <h2 className="text-3xl font-black leading-tight tracking-tight text-[#0D0D0D]">Spune-mi ce vrei.<br/>{t("euGasescCeChiar")}</h2>
            <p className="mt-2 text-sm font-medium text-[#6E6E80]">{t("aiulTauDeShopping")}</p>
            <div className="relative">
              <div className={`mt-4 flex items-center gap-2 rounded-xl p-3 ${THEME.classes.softInput}`}>
                ...
                <input aria-label={t("cautaPeSwypik")} value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") runRealSearch(input); }} className="w-full bg-transparent text-sm font-medium text-[#0D0D0D] outline-none placeholder:text-[#A1A1AA]" placeholder="Ex: setup gaming sub 4000, cadou creativ..." />
              </div>
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              <button type="button" onClick={() => runRealSearch(input)} disabled={!input.trim() || searchLoading} className="rounded-xl py-3.5 font-bold disabled:opacity-50 bg-white text-[#0D0D0D] border border-[#0D0D0D]">{t("cauta")}</button>
              <button type="button" onClick={() => sendMessage(input)} disabled={!input.trim() || isLoading} className="rounded-xl bg-[#0D0D0D] py-3.5 font-bold text-white disabled:opacity-50">{t("intreabaAi")}</button>
            </div>
          </div>
          <div className="mt-3 flex overflow-x-auto no-scrollbar gap-2">
            {getQuickPrompts(t).map((qa) => <button type="button" key={qa.label} onClick={() => sendMessage(qa.query)} className="shrink-0 whitespace-nowrap inline-flex items-center rounded-full bg-white border border-[#E5E5E5] px-4 py-2.5 text-xs font-bold text-[#0D0D0D] hover:border-[#0D0D0D] hover:text-[#0D0D0D] active:scale-95 transition-all min-h-[40px]">{qa.label}</button>)}
          </div>
          <div className="mt-4">
          ...
          {searchLoading ? <ProductCarousel title={t("seCautaRezultate")} isLoading={true} /> : searchResults.length > 0 && <ProductCarousel title={t("rezultateCount", { n: searchResults.length })} products={searchResults} />}
          <TrendingHero />
          <ProductCarousel title={t("alegerileCreatorilor")} products={topRatedProducts.slice(0, 10)} />
          <ProductCarousel title={t("calitatepretExcelent")} products={bestValueProducts.slice(0, 20)} />
          <BlogTeaser onOpenHub={() => setActiveTab("blog")} />
          <ProductCarousel title={t("topApreciateBadge")} products={topRatedProducts.slice(10, 30)} />
        </div>
      )}
      {activeTab === "blog" && <BlogHub />}
      {activeTab === "categories" && <div className="px-4 pt-4">{catBrowsing ? <div><div className="flex items-center gap-3 mb-4"><button type="button" onClick={() => { setCatBrowsing(false); setCatProducts([]); setCatTitle(""); setActiveCatId(null); }} className="grid h-10 w-10 place-items-center rounded-xl bg-[#F7F7F8] border border-[#E5E5E5]"><ChevronDown size={16} className="rotate-90" /></button><div className="flex-1"><p className="text-lg font-black text-[#0D0D0D]">{catTitle.split(" > ").map(p => translateCategory(p.trim())).join(" → ")}</p><p className="text-xs text-[#6E6E80]">{catTotal} {t("produseSuffix")}</p></div><button type="button" onClick={() => setShowFilters(!showFilters)} className={`flex items-center gap-1 rounded-xl px-3 py-2.5 text-xs font-bold border transition-all ${showFilters ? "bg-[#0D0D0D] text-white border-[#0D0D0D]" : "bg-white text-[#0D0D0D] border-[#E5E5E5]"}`}><SlidersHorizontal size={14} /></button></div>{showFilters && <div className="mb-4 rounded-xl bg-[#F7F7F8] p-4 border border-[#E5E5E5] animate-slideDown space-y-3"><div><label className="text-xs font-bold text-[#6E6E80] uppercase">{t("sortareLabel")}</label><div className="mt-1 flex flex-wrap gap-1.5">{[{v:"popular",l:t("sortPopular")},{v:"price_asc",l:t("sortPretAsc")},{v:"price_desc",l:t("sortPretDesc")},{v:"newest",l:t("sortNoi")},{v:"discount",l:t("sortReduceri")}].map(s=><button type="button" key={s.v} onClick={()=>{setFilterSort(s.v); browseCategory(catTitle, true, 1);}} className={`sub-chip chip-btn rounded-lg px-3.5 py-2 text-xs font-semibold ${filterSort===s.v?"active":""}`}>{s.l}</button>)}</div></div><div><label className="text-xs font-bold text-[#6E6E80] uppercase">{t("pretMaxim")} {filterMaxPrice} lei</label><input type="range" min={20} max={2000} step={10} value={filterMaxPrice} onChange={e=>{setFilterMaxPrice(Number(e.target.value));}} onMouseUp={()=>browseCategory(catTitle,true,1)} onTouchEnd={()=>browseCategory(catTitle,true,1)} className="mt-1 w-full accent-[#0D0D0D] h-8" /></div></div>}<div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 product-grid">{catProducts.map(p => <ProductCard key={p.id} product={p} />)}{catLoading && <><ProductSkeleton /><ProductSkeleton /><ProductSkeleton /><ProductSkeleton /></>}</div>{!catLoading && catProducts.length < catTotal && <button type="button" onClick={() => browseCategory(catTitle, true, catPage + 1)} className="mt-5 w-full rounded-xl bg-[#0D0D0D] py-4 text-sm font-bold text-white shadow-lg">{t("maiAfiseaza20Produse")} {catTotal})</button>}</div> : <><div className="flex items-center justify-between mb-4"><h2 className="text-2xl font-black text-[#0D0D0D]">Categorii</h2></div><div className="space-y-2">{(categoryTree as Array<{ id?: string; name: string; children?: Array<{ id?: string; name: string; children?: Array<{ id?: string; name: string; children?: Array<{ id?: string; name: string }> }> }> }>).map((cat)=><div key={cat.name} className="rounded-xl border border-[#E5E5E5] overflow-hidden"><button type="button" onClick={()=>{const hasChildren=(cat.children||[]).length>0; if((!hasChildren || expandedCat===cat.name) && cat.id){browseCategory(cat.name,true,1,cat.id);}else{setExpandedCat(expandedCat===cat.name?null:cat.name);}}} className={`w-full flex items-center justify-between p-4 text-left font-bold transition-colors ${expandedCat===cat.name?"bg-[#0D0D0D] text-white":"bg-white text-[#0D0D0D] hover:bg-[#F7F7F8]"}`}><span className="text-sm">{translateCategory(cat.name)}{expandedCat===cat.name && cat.id ? " — Vezi tot →" : ""}</span><div className="flex items-center gap-2">{/* count hidden — was hallucinated from upstream taxonomy, not actual inventory */}<ChevronDown size={16} className={`transition-transform ${expandedCat===cat.name?"rotate-180":""}`}/></div></button>{expandedCat===cat.name&&<div className="p-3 bg-[#FAFAFA] border-t border-[#E5E5E5] animate-slideDown"><div className="space-y-3">{(cat.children || []).map((mid)=><div key={mid.id || mid.name} className="rounded-xl border border-[#E5E5E5] bg-white p-3"><div className="flex items-center justify-between"><button type="button" onClick={()=>browseCategory(`${cat.name} > ${mid.name}`,true,1,mid.id)} className="text-xs font-black uppercase tracking-wide text-[#0D0D0D] hover:text-[#7C3AED] active:scale-95 transition-all text-left">{translateCategory(mid.name)} →</button><span className="text-[10px] font-bold text-[#6E6E80]">{/* count hidden */}</span></div><div className="mt-2 space-y-2">{(mid.children || []).map((sub)=><div key={sub.id || sub.name}><button type="button" onClick={()=>browseCategory(`${cat.name} > ${mid.name} > ${sub.name}`,true,1,sub.id)} className="mb-1 text-[11px] font-bold text-[#6E6E80] hover:text-[#0D0D0D] active:scale-95 transition-all text-left block">{translateCategory(sub.name)} →</button><div className="grid grid-cols-2 md:grid-cols-3 gap-2">{(sub.children || []).map((leaf)=><button type="button" key={leaf.id || leaf.name} onClick={()=>browseCategory(`${cat.name} > ${mid.name} > ${sub.name} > ${leaf.name}`,true,1,leaf.id)} className="rounded-xl p-3.5 text-left border-2 transition-all bg-white border-[#E5E5E5] hover:border-[#0D0D0D] active:scale-95"><span className="text-xs font-bold block">{translateCategory(leaf.name)}</span><span className="text-[10px] opacity-70">{/* count hidden */}</span></button>)}</div></div>)}</div></div>)}</div></div>}</div>)}</div></>}</div>}
      {activeTab === "chat" && <div className="px-4 pt-4"><div className="space-y-4">{(messages.length === 0 ? [getAiWelcome(t)] : messages).map((m) => <div key={m.id} className={m.role === "user" ? "text-right" : "text-left"}><div className={`inline-block max-w-[88%] rounded-2xl px-4 py-3 text-sm font-medium ${m.role === "user" ? "bg-[#0D0D0D] text-white" : "bg-[#F7F7F8] text-[#0D0D0D] border border-[#E5E5E5]"}`}>{m.role === "assistant" && <div className="mb-1 flex items-center gap-1 text-xs font-bold text-[#0D0D0D]"><Bot size={13} /> {t("asistentLabel")}</div>}<p className="whitespace-pre-wrap">{m.content}</p></div>{m.role === "assistant" && <><ProductCarousel title={t("recomandatePentruTine")} products={m.products} />{(m.bundleProducts?.length || 0) > 0 && <div className="mt-3 rounded-2xl border border-[#0D0D0D]/30 bg-gradient-to-br from-[#F0FDF4] to-[#ECFDF5] p-4"><div className="flex items-center justify-between mb-3"><p className="text-xs font-black uppercase tracking-widest text-[#0D0D0D]">{t("bundleAiCompleteazaSetul")}</p><p className="text-xs font-bold text-[#6E6E80]">{(() => { const total = (m.bundleProducts || []).reduce((s, p) => s + p.price, 0); const o = (m.bundleProducts || []).reduce((s, p) => s + (p.oldPrice || p.price), 0); return o > total ? t("economisesteLei", { amt: Math.round(o - total) }) : t("totalLei", { amt: Math.round(total) }); })()}</p></div><div className="space-y-2">{(m.bundleProducts || []).map(bp => <div key={bp.id} className="flex items-center gap-3 rounded-xl bg-white/80 p-2.5 border border-[#E5E5E5]/50"><Image src={bp.images?.[0]||""} alt="" width={48} height={48} className="h-12 w-12 rounded-lg object-cover shrink-0" /><div className="flex-1 min-w-0"><p className="text-xs font-bold text-[#0D0D0D] truncate">{bp.title}</p><p className="text-xs font-bold text-[#0D0D0D]">{bp.price} lei {bp.oldPrice > bp.price && <span className="text-[#A1A1AA] line-through ml-1">{bp.oldPrice}</span>}</p></div><button type="button" onClick={() => addToCart(bp)} className="shrink-0 rounded-lg bg-[#0D0D0D] px-2.5 py-1.5 text-[10px] font-black text-white active:scale-90 transition-transform">{t("cos")}</button></div>)}</div><button type="button" onClick={() => { (m.bundleProducts || []).forEach(bp => addToCart(bp)); setToastMessage(t("totBundleAdaugat")); setTimeout(() => setToastMessage(""), 2500); }} className="mt-3 w-full rounded-xl bg-[#0D0D0D] py-3 text-xs font-black text-white active:scale-95 transition-transform"><ShoppingCart size={13} className="inline mr-1.5" />{t("adaugaTotBundleul")} {Math.round((m.bundleProducts || []).reduce((s, p) => s + p.price, 0))} lei</button></div>}{(m.products?.length || 0) > 0 && <div className="mt-2 flex flex-wrap gap-1.5">{[{label:t("arataAltele"),query:t("promptArataAltceva")},{label:t("maiIeftin"),query:t("promptMaiIeftin")},{label:t("topCalitate"),query:t("promptDoarTopCalitate")},{label:t("maiNoi"),query:t("promptProduseNoi")},{label:t("comparaTop2"),query:t("promptComparaTop2")}].map(chip => <button type="button" key={chip.label} onClick={() => sendMessage(chip.query)} className="rounded-full bg-white border border-[#E5E5E5] px-3 py-1.5 text-[11px] font-bold text-[#6E6E80] hover:border-[#0D0D0D] hover:text-[#0D0D0D] active:scale-95 transition-all">{chip.label}</button>)}</div>}</>}</div>)}{isLoading && <div className="rounded-xl bg-[#F7F7F8] p-3 text-sm font-medium text-[#6E6E80] border border-[#E5E5E5]">{t("aiAnalizeazaSiCauta")}</div>}{messages.length > 0 && !isLoading && <button type="button" onClick={() => { setMessages([]); try { localStorage.removeItem("aicv_chat"); } catch {} }} className="mx-auto block text-[10px] font-bold text-[#A1A1AA] hover:text-[#6E6E80] mt-2">{t("stergeConversatia")}</button>}<div ref={messagesEndRef} /></div></div>}
      {activeTab === "deals" && <div className="px-4 pt-4"><h2 className="mb-3 text-2xl font-black text-[#0D0D0D]">{t("reduceriTab")}</h2>{dealsLoading ? <p className="py-20 text-center font-medium text-[#6E6E80]">{t("seIncarca")}</p> : <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 product-grid">{dealsProducts.map((p) => <ProductCard key={p.id} product={p} />)}</div>}</div>}
      {activeTab === "feed" && <ProductFeed products={feedProducts} onAddToCart={(p, q?: number) => addToCart(p as unknown as ChatProduct, q || 1)} onLoadMore={loadMoreFeed} onClose={() => setActiveTab("home")} isLoading={feedLoading} />}
      {activeTab === "cart" && (
            ...
            <>
              ...
              <div className="mt-5 rounded-2xl bg-[#F7F7F8] p-4 border border-[#E5E5E5]">
                ...
                <button type="button" onClick={submitOrder} disabled={checkoutLoading} className={`mt-4 w-full rounded-xl py-4 font-bold disabled:opacity-50 ${THEME.classes.cartButton} active:scale-[0.98] transition-transform`}>
                  {checkoutLoading ? t("seProceseazaDots") : t("finalizeazaComandaLei", { amt: cartTotal })}
                </button>
              </div>
            </>
          )}
    ...
    {activeTab === "chat" && <div className="fixed z-30 w-full md:max-w-2xl lg:max-w-4xl xl:max-w-6xl left-1/2 -translate-x-1/2 border-t border-[#E5E5E5] bg-white/95 px-3 py-2 backdrop-blur-xl chat-input-bar" onTouchStart={e=>e.stopPropagation()} onTouchEnd={e=>e.stopPropagation()}><div className={`flex gap-2 rounded-xl p-2 ${THEME.classes.softInput}`}><input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && sendMessage()} className="flex-1 bg-transparent px-2 text-base font-medium text-[#0D0D0D] outline-none placeholder:text-[#A1A1AA]" placeholder={t("scrieCeCauti")} /><button type="button" onClick={() => sendMessage()} disabled={!input.trim() || isLoading} className="grid h-11 w-11 place-items-center rounded-xl bg-[#0D0D0D] text-white disabled:opacity-40 focus-visible:ring-2 focus-visible:ring-violet-500 focus-visible:outline-none" aria-label={t("ariaTrimite")}><Send size={18} /></button></div></div>}{(activeTab as string) !== "feed" && <nav className="fixed bottom-0 left-1/2 z-40 w-full md:max-w-2xl lg:max-w-4xl xl:max-w-6xl -translate-x-1/2 border-t border-[#E5E5E5] bg-white/95 px-3 py-2 pb-[max(8px,env(safe-area-inset-bottom))] backdrop-blur-xl bottom-nav"><div className="grid grid-cols-7 items-center text-[10px] font-bold text-[#6E6E80]"><NavBtn icon={<Home size={20} />} label={t("navAcasa")} active={activeTab === "home"} onClick={() => setActiveTab("home")} /><NavBtn icon={<Grid3x3 size={20} />} label={t("navCategorii")} active={activeTab === "categories"} onClick={() => { setActiveTab("categories"); setCatBrowsing(false); }} /><NavBtn icon={<BookOpen size={20} />} label={t("navGhiduri")} active={activeTab === "blog"} onClick={() => setActiveTab("blog")} /><NavBtn icon={<Compass size={20} />} label={t("navFeed")} active={false} onClick={() => router.push("/explore")} /><Link href="/reels/record" aria-label={t("incarca")} className="mx-auto -mt-4 flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br from-[#7C3AED] to-[#A855F7] text-white shadow-lg ring-4 ring-white active:scale-95 transition-transform focus-visible:ring-2 focus-visible:ring-violet-500 focus-visible:outline-none"><Plus size={26} strokeWidth={2.6} /></Link><NavBtn icon={<MessageCircle size={20} />} label={t("navChat")} active={activeTab === "chat"} onClick={() => setActiveTab("chat")} /><NavBtn icon={<ShoppingCart size={20} />} label={`${t("navCos")} ${cartCount ? `(${cartCount})` : ""}`} active={false} onClick={() => router.push("/cart")} /></div></nav>}
...
function ProductModal({ product, onClose, onAddToCart }: { product: ChatProduct; onClose: () => void; onAddToCart: () => void }) { const t = useTranslations("chatInterface"); const insights: string[] = []; if (product.rating >= 4.7 && !product.isEstimatedSocial) insights.push(t('insightRatingPesteMedie', { rating: product.rating })); if (!product.isEstimatedSocial && product.orders >= 500) insights.push(t('insightComenziSellerIncredere', { orders: product.orders.toLocaleString() })); else if (!product.isEstimatedSocial && product.orders >= 100) insights.push(t('insightVanduteVerificat', { orders: product.orders.toLocaleString() })); if (product.discountPercent >= 20) insights.push(t('insightReducereRealaPct', { pct: product.discountPercent })); if (product.qualityScore >= 9) insights.push(t('insightBestValueCategorie')); if (product.deliveryDays <= 5) insights.push(t('insightLivrareRapidaZile', { days: product.deliveryDays })); return <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/30" onClick={onClose}><div className="max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-t-[2rem] bg-white p-5" onClick={(e) => e.stopPropagation()}><div className="mb-3 flex justify-between"><span className="rounded-full bg-[#F7F7F8] px-3 py-1 text-xs font-bold text-[#6E6E80]">{product.category}</span><button type="button" onClick={onClose} aria-label={t("inchide4")}><X size={18} /></button></div>{product.images?.[0] && <Image src={product.images[0]} alt={product.title} width={640} height={640} className="h-64 w-full rounded-2xl object-cover" />}<h2 className="mt-4 text-2xl font-black text-[#0D0D0D]">{product.title}</h2><div className="mt-2 flex gap-3 text-sm font-medium text-[#6E6E80]"><span className="text-[#B45309]"><Star size={14} className="inline" fill="currentColor" /> {product.rating}</span>{product.socialProofLabel ? <span>{product.socialProofLabel}</span> : <span>{t("popularBadge")}</span>}<span><Truck size={14} className="inline" /> {product.deliveryDays} {t("zileLivrare")}</span></div><div className="mt-3"><span className="text-3xl font-black text-[#0D0D0D]">{product.price} lei</span>{product.oldPrice > product.price && <span className="ml-2 text-[#6E6E80] line-through">{product.oldPrice} lei</span>}</div>{insights.length > 0 && <div className="mt-4 rounded-2xl bg-gradient-to-br from-[#F0FDF4] to-[#ECFDF5] border border-[#BBF7D0] p-4"><p className="text-xs font-black uppercase tracking-widest text-[#0D0D0D] mb-2">{t("deCeMeritaAi")}</p><div className="space-y-1.5">{insights.map((ins, i) => <p key={i} className="text-sm font-medium text-[#0D0D0D]">{ins}</p>)}</div></div>}<p className="mt-4 text-sm font-medium leading-relaxed text-[#6E6E80]">{product.description}</p><button type="button" onClick={onAddToCart} className={`mt-4 w-full rounded-xl py-4 font-bold ${THEME.classes.cartButton}`}>{t("adaugaInCos")} {product.price} lei</button><a href={`/product/${product.pgId || product.id}`} className="mt-2 block w-full rounded-xl border border-[#E5E5E5] bg-[#F7F7F8] py-3 text-center text-sm font-bold text-[#0D0D0D] hover:bg-[#ECECF1] transition-colors">{t("veziToateDetaliile")}</a></div></div>; }
