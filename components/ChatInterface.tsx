...
export default function ChatInterface() {
  ...
  const ProductCarousel = ({ title, products }: { title: string; products?: ChatProduct[] }) => !products?.length ? null : <div className="mt-4 text-left"><p className="mb-2 text-xs font-bold uppercase tracking-widest text-[#6E6E80]">{title}</p><div className="flex snap-x gap-3 overflow-x-auto pb-3 no-scrollbar">{products.map((p) => <div key={p.id} className="snap-start"><ProductCard product={p} compact /></div>)}</div></div>;
