...
export default function ChatInterface() {
  ...
  const ProductCard = ({ product, compact = false }: { product: ChatProduct; compact?: boolean }) => { const badge = product.commerceBadge; const insight = product.rating >= 4.8 && product.orders >= 200 ? '⭐ Calitate peste medie' : product.orders >= 500 ? '✅ Seller verificat' : product.discountPercent >= 25 ? '💰 Reducere reală' : product.qualityScore >= 9 ? '🏆 Best value' : null; return <div className={`${compact ? "w-48 shrink-0" : ""} overflow-hidden rounded-2xl bg-white border border-[#E5E5E5] hover:border-[#D1D1D6] hover:shadow-md transition-all cursor-pointer`} onClick={() => setSelectedProduct(product)}><div className="relative h-40 bg-[#F7F7F8]">{product.images?.[0] ? <img src={product.images[0]} alt={product.title} className="h-full w-full object-cover" /> : <div className="grid h-full place-items-center"><Package className="text-[#D1D1D6]" /></div>}{product.discountPercent > 0 && <span className="absolute right-2 top-2 rounded-full bg-[#EF4444] px-2.5 py-1 text-[10px] font-black text-white">-{product.discountPercent}%</span>}{badge && <span className={`absolute left-2 top-2 max-w-[80%] rounded-full px-2.5 py-1 text-[10px] font-black shadow ${commerceBadgeClass(badge)}`}>{badge}</span>}</div><div className="p-3"><p className="line-clamp-2 text-sm font-bold leading-tight text-[#0D0D0D]">{product.title}</p>{insight && <p className="mt-1 text-[11px] font-semibold text-[#10A37F]">{insight}</p>}<div className="mt-1 flex items-center gap-2 text-[11px] font-medium text-[#6E6E80]"><span className="text-[#F59E0B]">★ {product.rating?.toFixed?.(1) || "4.8"}</span><span>{product.orders || 0}+ comenzi</span></div><div className="mt-2 flex items-end gap-2"><span className="text-xl font-black text-[#10A37F]">{product.price} lei</span>{product.oldPrice > product.price && <span className="text-xs text-[#6E6E80] line-through">{product.oldPrice} lei</span>}</div><button onClick={(e) => { e.stopPropagation(); addToCart(product); }} className={`mt-3 w-full rounded-xl py-2.5 text-sm font-bold ${THEME.classes.cartButton}`}>Adaugă în coș</button></div></div>; };
