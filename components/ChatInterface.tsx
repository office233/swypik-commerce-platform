...
export default function ChatInterface() {
  ...
  useEffect(() => { setSessionId(Math.random().toString(36).slice(2) + Date.now().toString(36)); fetch("/api/products?mode=trending&limit=30").then((r) => r.json()).then((d) => setTrendingProducts(d.products || [])).catch(() => {}); fetch("/api/products?mode=bestvalue&limit=30").then((r) => r.json()).then((d) => setBestValueProducts(d.products || [])).catch(() => {}); fetch("/api/products?mode=toprated&limit=30").then((r) => r.json()).then((d) => setTopRatedProducts(d.products || [])).catch(() => {}); fetch("/api/products?hierarchy=true").then(r=>r.json()).then(d=>setCategoryTree(d.hierarchy||[])).catch(()=>{}); /* Auto-open feed from URL */ if (typeof window !== "undefined") { const urlMode = new URLSearchParams(window.location.search).get("mode"); if (urlMode === "feed") { setActiveTab("feed"); loadFeed(); } } }, []);
  ...
  async function loadFeed() { if (feedLoading) return; setFeedLoading(true); try { const data = await fetch("/api/products?mode=video&limit=15&sort=popular").then((r) => r.json()); setFeedProducts(data.products || []); } finally { setFeedLoading(false); } }
  const [touchStart, setTouchStart] = useState<{ x: number, y: number } | null>(null);

  async function loadMoreFeed() { if (feedLoading) return; setFeedLoading(true); try { const offset = feedProducts.length; const data = await fetch(`/api/products?mode=video&limit=15&offset=${offset}&sort=popular`).then((r) => r.json()); setFeedProducts((prev) => { const existing = new Set(prev.map((p) => p.id)); return [...prev, ...(data.products || []).filter((p: ChatProduct) => !existing.has(p.id))]; }); } finally { setFeedLoading(false); } }
