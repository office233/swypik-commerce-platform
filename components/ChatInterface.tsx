...
type ChatProduct = {
  id: string; title: string; description: string; benefits: string[]; dealLabel: string; whyBuy: string; warnings: string[];
...
export default function ChatInterface() {
  ...
  useEffect(() => { setSessionId(crypto.randomUUID()); fetch("/api/shopify-products?mode=trending&limit=20").then((r) => r.json()).then((d) => setTrendingProducts(d.products || [])).catch(() => {}); }, []);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      fetch(`/api/search/suggest?q=${encodeURIComponent(input.trim())}&limit=8`, { signal: ctrl.signal })
        .then((r) => r.json())
  ...
  function addToCart(product: ChatProduct) {
    ...
    setSelectedProduct(null); setToastMessage(`🛒 ${product.title.slice(0, 24)} adăugat în coș`); setTimeout(() => setToastMessage(""), 2500);
  }
  ...
  async function runRealSearch(query: string) {
    ...
    setSearchLoading(true); setSuggestions([]); setInput(q);
    try {
      const res = await fetch(`/api/shopify-products?q=${encodeURIComponent(q)}&limit=24&requireImage=true`);
  ...
  async function submitOrder() { if (cartItems.length === 0 || checkoutLoading) return; if (!checkoutForm.name || !checkoutForm.phone || !checkoutForm.address || !checkoutForm.city) { setToastMessage("Completează nume, telefon, adresă și oraș."); setTimeout(() => setToastMessage(""), 3000); return; } setCheckoutLoading(true); try { const res = await fetch("/api/cart", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ products: cartItems.map((item) => ({ ...item.product, quantity: item.qty })), customer: checkoutForm }) }); const data = await res.json(); if (!res.ok || !data.success) throw new Error(data?.error || "Nu am putut crea checkout-ul."); if (data.checkoutUrl) window.location.href = data.checkoutUrl; } catch (error: any) { setToastMessage(error?.message || "Eroare checkout."); setTimeout(() => setToastMessage(""), 4000); } finally { setCheckoutLoading(false); } }

  const ProductCard = ({ product, compact = false }: { product: ChatProduct; compact?: boolean }) => { const badge = product.commerceBadge || product.socialProofLabel; return <div className={`${compact ? "w-48 shrink-0" : ""} overflow-hidden rounded-[1.6rem] ${THEME.classes.card}`} onClick={() => setSelectedProduct(product)}><div className="relative h-40 bg-orange-50">{product.images?.[0] ? <img src={product.images[0]} alt={product.title} className="h-full w-full object-cover" /> : <div className="grid h-full place-items-center"><Package className="text-orange-300" /></div>}{product.discountPercent > 0 && <span className={`absolute right-2 top-2 rounded-full px-2.5 py-1 text-[10px] font-black ${THEME.classes.discountBadge}`}>-{product.discountPercent}%</span>}{badge && <span className={`absolute left-2 top-2 max-w-[80%] rounded-full px-2.5 py-1 text-[10px] font-black shadow ${commerceBadgeClass(badge)}`}>{badge}</span>}</div><div className="p-3"><p className="line-clamp-2 text-sm font-black leading-tight text-slate-900">{product.title}</p><div className="mt-1 flex items-center gap-2 text-[11px] font-bold text-slate-500"><span className="text-amber-500">★ {product.rating?.toFixed?.(1) || "4.8"}</span><span>{product.orders || 0}+ comenzi</span></div><div className="mt-2 flex items-end gap-2"><span className="text-xl font-black text-[#FF5A1F]">{product.price} lei</span>{product.oldPrice > product.price && <span className="text-xs text-slate-400 line-through">{product.oldPrice} lei</span>}</div><button onClick={(e) => { e.stopPropagation(); addToCart(product); }} className={`mt-3 w-full rounded-2xl py-2.5 text-sm font-black ${THEME.classes.cartButton}`}>🛒 + Coș</button></div></div>; };
  const ProductCarousel = ({ title, products }: { title: string; products?: ChatProduct[] }) => !products?.length ? null : <div className="mt-4 text-left"><p className="mb-2 text-xs font-black uppercase tracking-widest text-slate-500">{title}</p><div className="flex snap-x gap-3 overflow-x-auto pb-3">{products.map((p) => <div key={p.id} className="snap-start"><ProductCard product={p} compact /></div>)}</div></div>;
  ...
  return <main className={`min-h-screen ${THEME.classes.appBg}`}><div className={`relative mx-auto min-h-screen max-w-lg ${THEME.classes.pageBg}`}>{activeTab !== "feed" && <header className="sticky top-0 z-30 border-b border-orange-100 bg-white/90 px-4 py-3 shadow-sm backdrop-blur-xl"><div className="flex items-center justify-between"><button onClick={() => setActiveTab("home")} className="flex items-center gap-2"><span className={`grid h-10 w-10 place-items-center rounded-2xl text-sm font-black ${THEME.classes.primaryButton}`}>AI</span><div className="text-left"><h1 className="text-lg font-black text-slate-950">AICeVrei.ro</h1><p className="text-[10px] font-bold text-orange-500">AI shopping seller</p></div></button><button onClick={() => setActiveTab("cart")} className={`relative rounded-full p-2.5 ${THEME.classes.primaryButton}`}><ShoppingCart size={18} />{cartCount > 0 && <span className="absolute -right-1 -top-1 grid h-5 w-5 place-items-center rounded-full bg-[#16A34A] text-[10px] font-black text-white">{cartCount}</span>}</button></div></header>}
    <section className={activeTab === "feed" ? "h-screen" : "min-h-[calc(100vh-132px)] pb-36"}>{activeTab === "home" && <div className="px-4 pt-6"><div className={`rounded-[2rem] p-6 ${THEME.classes.heroCard}`}><div className="mb-4 inline-flex items-center gap-2 rounded-full bg-orange-100 px-3 py-1.5 text-xs font-black text-orange-600"><Zap size={14} /> Search real din Shopify</div><h2 className="text-4xl font-black leading-none tracking-tight text-slate-950">Caută orice produs. AI-ul îți face și bundle.</h2><p className="mt-3 text-sm font-semibold text-slate-500">Search instant, filtre reale și produse direct din Shopify.</p><div className="relative"><div className={`mt-5 flex items-center gap-2 rounded-2xl p-3 ${THEME.classes.softInput}`}><Search size={20} className="text-orange-500" /><input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") runRealSearch(input); }} className="w-full bg-transparent text-sm font-bold text-slate-900 outline-none placeholder:text-slate-400" placeholder="Ex: rochie, cadou, sub 100" /></div>{suggestions.length > 0 && <div className="absolute left-0 right-0 top-full z-40 mt-2 overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-orange-100">{suggestions.map((s) => <button key={`${s.type}-${s.label}`} onClick={() => runRealSearch(s.label)} className="flex w-full items-center justify-between px-4 py-3 text-left text-sm font-bold text-slate-800 hover:bg-orange-50"><span>{s.label}</span><span className="rounded-full bg-orange-100 px-2 py-0.5 text-[10px] font-black text-orange-600">{s.type}</span></button>)}</div>}</div><div className="mt-3 grid grid-cols-2 gap-2"><button onClick={() => runRealSearch(input)} disabled={!input.trim() || searchLoading} className={`rounded-2xl py-4 font-black disabled:opacity-50 ${THEME.classes.primaryButton}`}>🔍 Caută</button><button onClick={() => sendMessage(input)} disabled={!input.trim() || isLoading} className="rounded-2xl bg-slate-900 py-4 font-black text-white disabled:opacity-50">🤖 Întreabă AI</button></div></div><div className="mt-5 grid grid-cols-3 gap-2">{QUICK_ACTIONS.map((a) => <button key={a.label} onClick={() => runRealSearch(a.query)} className="rounded-2xl bg-white px-2 py-3 text-xs font-black text-slate-800 shadow-sm ring-1 ring-orange-100 active:scale-95">{a.label}</button>)}</div>{searchLoading && <p className="mt-5 rounded-2xl bg-white p-4 text-center text-sm font-black text-slate-500 shadow-sm">Caut produse bune în Shopify...</p>}{searchResults.length > 0 && <ProductCarousel title={`🔍 Rezultate (${searchResults.length})`} products={searchResults} />}<ProductCarousel title="🔥 Trending acum" products={trendingProducts.slice(0, 10)} /></div>}
