...
export default function ChatInterface({ 
  ...
  return <main className={`min-h-screen ${THEME.classes.appBg}`}><div className={`relative mx-auto min-h-screen w-full md:max-w-2xl lg:max-w-4xl xl:max-w-6xl app-container ${THEME.classes.pageBg}`}>{activeTab !== "feed" && <header className="sticky top-0 z-30 border-b border-[#E5E5E5] bg-white/95 px-4 py-3 backdrop-blur-xl safe-top"><div className="flex items-center justify-between h-12"><button type="button" onClick={() => { setActiveTab("home"); setCatBrowsing(false); setExpandedCat(null); if (typeof window !== "undefined") window.scrollTo({ top: 0, behavior: "smooth" }); }} className="flex items-center gap-2" aria-label="Înapoi acasă"><span className="text-2xl font-black text-[#0D0D0D] tracking-tight">Swypik</span></button><div className="flex items-center gap-2"><a href="/cart" aria-label="Coșul meu" className="relative grid h-11 w-11 place-items-center rounded-full bg-[#0D0D0D] text-white active:scale-95 transition-transform focus-visible:ring-2 focus-visible:ring-violet-500 focus-visible:outline-none"><ShoppingCart size={18} />{cartCount > 0 && <span className="absolute top-0.5 right-0.5 grid h-[15px] min-w-[15px] place-items-center rounded-full bg-[#7C3AED] px-1 text-[9px] font-black text-white leading-none ring-2 ring-[#0D0D0D]">{cartCount > 99 ? "99+" : cartCount}</span>}</a></div></div></header>}
    {/* Slide-out Menu */}
    {showMenu && <div className="fixed inset-0 z-50" role="dialog" aria-modal="true" aria-label="Meniu navigare" onClick={() => setShowMenu(false)}>
      ...
      <div className="absolute right-0 top-0 h-full w-72 bg-white shadow-2xl" onClick={e => e.stopPropagation()} style={{animation: 'slideInRight 0.2s ease-out'}}>
        <div className="p-5 border-b border-[#E5E5E5] flex items-center justify-between">
          ...
          <button onClick={() => setShowMenu(false)} className="rounded-lg p-1.5 hover:bg-[#F7F7F8] transition" aria-label="Închide"><X size={20} /></button>
        </div>
    ...
    <section className={activeTab === "feed" ? "h-[100dvh]" : "min-h-[calc(100dvh-132px)] pb-20"} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
      {activeTab === "home" && (
        <div className="px-4 pt-6">
          <div className="rounded-2xl border border-[#E5E5E5] bg-[#F7F7F8] p-6">
            ...
            <div className="relative">
              <div className={`mt-4 flex items-center gap-2 rounded-xl p-3 ${THEME.classes.softInput}`}>
                ...
                <input aria-label="Caută pe Swypik" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") runRealSearch(input); }} className="w-full bg-transparent text-sm font-medium text-[#0D0D0D] outline-none placeholder:text-[#A1A1AA]" placeholder="Ex: setup gaming sub 4000, cadou creativ..." />
              </div>
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              <button type="button" onClick={() => runRealSearch(input)} disabled={!input.trim() || searchLoading} className="rounded-xl py-3.5 font-bold disabled:opacity-50 bg-white text-[#0D0D0D] border border-[#0D0D0D]">Caută</button>
          ...
          <ProductCarousel title="🏆 Calitate-preț excelent" products={bestValueProducts.slice(0, 20)} />
      ...
      {activeTab === "chat" && <div className="px-4 pt-4"><div className="space-y-4">{(messages.length === 0 ? [AI_WELCOME] : messages).map((m) => <div key={m.id} className={m.role === "user" ? "text-right" : "text-left"}><div className={`inline-block max-w-[88%] rounded-2xl px-4 py-3 text-sm font-medium ${m.role === "user" ? "bg-[#0D0D0D] text-white" : "bg-[#F7F7F8] text-[#0D0D0D] border border-[#E5E5E5]"}`}>{m.role === "assistant" && <div className="mb-1 flex items-center gap-1 text-xs font-bold text-[#0D0D0D]"><Bot size={13} /> Asistent Shopping AI</div>}<p className="whitespace-pre-wrap">{m.content}</p></div>{m.role === "assistant" && <><ProductCarousel title="Recomandate pentru tine" products={m.products} />{(m.bundleProducts?.length || 0) > 0 && <div className="mt-3 rounded-2xl border border-[#0D0D0D]/30 bg-gradient-to-br from-[#F0FDF4] to-[#ECFDF5] p-4"><div className="flex items-center justify-between mb-3"><p className="text-xs font-black uppercase tracking-widest text-[#0D0D0D]">🎁 Bundle AI — Completează setul</p><p className="text-xs font-bold text-[#6E6E80]">{(() => { const t = (m.bundleProducts || []).reduce((s, p) => s + p.price, 0); const o = (m.bundleProducts || []).reduce((s, p) => s + (p.oldPrice || p.price), 0); return o > t ? `Economisești ${Math.round(o - t)} lei` : `Total: ${Math.round(t)} lei`; })()}</p></div><div className="space-y-2">{(m.bundleProducts || []).map(bp => <div key={bp.id} className="flex items-center gap-3 rounded-xl bg-white/80 p-2.5 border border-[#E5E5E5]/50"><Image src={bp.images?.[0]||""} alt="" width={48} height={48} className="h-12 w-12 rounded-lg object-cover shrink-0" /><div className="flex-1 min-w-0"><p className="text-xs font-bold text-[#0D0D0D] truncate">{bp.title}</p><p className="text-xs font-bold text-[#0D0D0D]">{bp.price} lei {bp.oldPrice > bp.price && <span className="text-[#A1A1AA] line-through ml-1">{bp.oldPrice}</span>}</p></div><button type="button" onClick={() => addToCart(bp)} className="shrink-0 rounded-lg bg-[#0D0D0D] px-2.5 py-1.5 text-[10px] font-black text-white active:scale-90 transition-transform">+ Coș</button></div>)}</div><button type="button" onClick={() => { (m.bundleProducts || []).forEach(bp => addToCart(bp)); setToastMessage("🎁 Tot bundle-ul adăugat!"); setTimeout(() => setToastMessage(""), 2500); }} className="mt-3 w-full rounded-xl bg-[#0D0D0D] py-3 text-xs font-black text-white active:scale-95 transition-transform"><ShoppingCart size={13} className="inline mr-1.5" />Adaugă tot bundle-ul — {Math.round((m.bundleProducts || []).reduce((s, p) => s + p.price, 0))} lei</button></div>}{(m.products?.length || 0) > 0 && <div className="mt-2 flex flex-wrap gap-1.5">{[{label:"🔄 Arată altele",query:"arată-mi altceva"},{label:"💰 Mai ieftin",query:"vreau mai ieftin"},{label:"⭐ Top calitate",query:"arată-mi doar top calitate"},{label:"🆕 Mai noi",query:"arată-mi produse mai noi"},{label:"⚖️ Compară top 2",query:"compară primele 2 produse"}].map(chip => <button type="button" key={chip.label} onClick={() => sendMessage(chip.query)} className="rounded-full bg-white border border-[#E5E5E5] px-3 py-1.5 text-[11px] font-bold text-[#6E6E80] hover:border-[#0D0D0D] hover:text-[#0D0D0D] active:scale-95 transition-all">{chip.label}</button>)}</div>}</>}</div>)}{isLoading && <div className="rounded-xl bg-[#F7F7F8] p-3 text-sm font-medium text-[#6E6E80] border border-[#E5E5E5]">🧠 AI analizează și caută cele mai bune opțiuni...</div>}{messages.length > 0 && !isLoading && <button type="button" onClick={() => { setMessages([]); try { localStorage.removeItem("aicv_chat"); } catch {} }} className="mx-auto block text-[10px] font-bold text-[#A1A1AA] hover:text-[#6E6E80] mt-2">🗑️ Șterge conversația</button>}<div ref={messagesEndRef} /></div></div>}
