...
export default function ChatInterface() {
  ...
  const ProductCard = ({ product, compact = false }: { product: ChatProduct; compact?: boolean }) => (
    <div className={`${compact ? "w-48 shrink-0" : ""} overflow-hidden rounded-[1.6rem] border border-orange-100 bg-white shadow-[0_18px_45px_rgba(15,23,42,0.10)]`} onClick={() => setSelectedProduct(product)}>
      <div className="relative h-40 bg-orange-50">
        ...
        {product.discountPercent > 0 && <span className="absolute right-2 top-2 rounded-full bg-[#ff4d4d] px-2.5 py-1 text-[10px] font-black text-white shadow-lg">-{product.discountPercent}%</span>}
        {(product.commerceBadge || product.socialProofLabel) && <span className="absolute left-2 top-2 max-w-[80%] rounded-full bg-white/95 px-2.5 py-1 text-[10px] font-black text-[#f97316] shadow">{product.commerceBadge || product.socialProofLabel}</span>}
      </div>
      <div className="p-3">
        ...
        <div className="mt-2 flex items-end gap-2"><span className="text-xl font-black text-[#ff4d2e]">{product.price} lei</span>{product.oldPrice > product.price && <span className="text-xs text-slate-400 line-through">{product.oldPrice} lei</span>}</div>
        <button onClick={(e) => { e.stopPropagation(); addToCart(product); }} className="mt-3 w-full rounded-2xl bg-[#22c55e] py-2.5 text-sm font-black text-white shadow-lg shadow-green-500/20 active:scale-95">🛒 + Coș</button>
      </div>
    </div>
  );
  ...
  return (
    <main className="min-h-screen bg-[#fff7ed] text-slate-950">
      <div className="relative mx-auto min-h-screen max-w-lg bg-[linear-gradient(180deg,#fff7ed_0%,#fffaf5_45%,#ffffff_100%)]">
        {activeTab !== "feed" && <header className="sticky top-0 z-30 border-b border-orange-100 bg-white/90 px-4 py-3 shadow-sm backdrop-blur-xl"><div className="flex items-center justify-between"><button onClick={() => setActiveTab("home")} className="flex items-center gap-2"><span className="grid h-10 w-10 place-items-center rounded-2xl bg-[#ff6b35] text-sm font-black text-white shadow-lg shadow-orange-500/30">AI</span><div className="text-left"><h1 className="text-lg font-black text-slate-950">AICeVrei.ro</h1><p className="text-[10px] font-bold text-orange-500">AI shopping seller</p></div></button><button onClick={() => setActiveTab("cart")} className="relative rounded-full bg-[#ff6b35] p-2.5 text-white shadow-lg shadow-orange-500/30"><ShoppingCart size={18} />{cartCount > 0 && <span className="absolute -right-1 -top-1 grid h-5 w-5 place-items-center rounded-full bg-[#22c55e] text-[10px] font-black text-white">{cartCount}</span>}</button></div></header>}

        <section className={activeTab === "feed" ? "h-screen" : "min-h-[calc(100vh-132px)] pb-36"}>
          {activeTab === "home" && <div className="px-4 pt-6"><div className="rounded-[2rem] bg-white p-6 shadow-[0_25px_70px_rgba(249,115,22,0.18)]"><div className="mb-4 inline-flex items-center gap-2 rounded-full bg-orange-100 px-3 py-1.5 text-xs font-black text-orange-600"><Zap size={14} /> AI Seller + Bundle Builder</div><h2 className="text-4xl font-black leading-none tracking-tight text-slate-950">Spune-mi ce vrei și îți fac coșul perfect.</h2><p className="mt-3 text-sm font-semibold text-slate-500">Recomandări, bundle-uri și checkout rapid direct din Shopify.</p><div className="mt-5 flex items-center gap-2 rounded-2xl border-2 border-orange-100 bg-orange-50 p-3"><Search size={20} className="text-orange-500" /><input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && sendMessage()} className="w-full bg-transparent text-sm font-bold text-slate-900 outline-none placeholder:text-slate-400" placeholder="Ex: cadou pentru iubita" /></div><button onClick={() => sendMessage()} disabled={!input.trim() || isLoading} className="mt-3 w-full rounded-2xl bg-[#ff6b35] py-4 font-black text-white shadow-xl shadow-orange-500/25 disabled:opacity-50">🔥 Caută și fă bundle</button></div><div className="mt-5 grid grid-cols-3 gap-2">{QUICK_ACTIONS.map((a) => <button key={a.label} onClick={() => sendMessage(a.query)} className="rounded-2xl bg-white px-2 py-3 text-xs font-black text-slate-800 shadow-sm ring-1 ring-orange-100 active:scale-95">{a.label}</button>)}</div><ProductCarousel title="🔥 Trending acum" products={trendingProducts.slice(0, 10)} /></div>}
          {activeTab === "chat" && <div className="px-4 pt-4"><div className="space-y-4">{messages.map((m) => <div key={m.id} className={m.role === "user" ? "text-right" : "text-left"}><div className={`inline-block max-w-[88%] rounded-3xl px-4 py-3 text-sm font-semibold shadow-sm ${m.role === "user" ? "bg-[#ff6b35] text-white" : "bg-white text-slate-900 ring-1 ring-orange-100"}`}>{m.role === "assistant" && <div className="mb-1 flex items-center gap-1 text-xs font-black text-orange-500"><Bot size={13} /> AI Seller</div>}<p className="whitespace-pre-wrap">{m.content}</p></div>{m.role === "assistant" && <><ProductCarousel title="🎯 Recomandate pentru tine" products={m.products} /><ProductCarousel title="🔥 Completează bundle-ul" products={m.bundleProducts} /></>}</div>)}{isLoading && <div className="rounded-2xl bg-white p-3 text-sm font-bold text-slate-500 shadow-sm">AI construiește recomandarea...</div>}<div ref={messagesEndRef} /></div></div>}
          ...
          {activeTab === "cart" && <div className="px-4 pt-4"><h2 className="mb-4 text-2xl font-black text-slate-950">🛒 Coșul tău</h2>{cartItems.length === 0 ? <p className="py-20 text-center font-bold text-slate-400">Coșul este gol.</p> : <><div className="space-y-3">{cartItems.map((item, i) => <div key={item.product.id} className="flex gap-3 rounded-3xl bg-white p-3 shadow-sm ring-1 ring-orange-100">{item.product.images?.[0] && <img src={item.product.images[0]} alt="" className="h-16 w-16 rounded-2xl object-cover" />}<div className="flex-1"><p className="line-clamp-2 text-sm font-black text-slate-900">{item.product.title}</p><p className="text-xs font-black text-[#ff4d2e]">{item.product.price * item.qty} lei</p></div><div className="flex items-center gap-2"><button onClick={() => updateQty(i, -1)} className="rounded-full bg-orange-50 px-2 font-black text-orange-600">-</button><span className="font-black">{item.qty}</span><button onClick={() => updateQty(i, 1)} className="rounded-full bg-orange-50 px-2 font-black text-orange-600">+</button></div></div>)}</div><div className="mt-5 rounded-3xl bg-white p-4 shadow-lg ring-1 ring-orange-100"><div className="flex justify-between text-xl font-black"><span>Total</span><span className="text-[#ff4d2e]">{cartTotal} lei</span></div><button onClick={() => setShowCheckoutForm(true)} className="mt-4 w-full rounded-2xl bg-[#22c55e] py-4 font-black text-white shadow-xl shadow-green-500/20">Finalizează comanda</button></div></>}</div>}
        </section>

        {activeTab !== "feed" && <div className="fixed bottom-16 left-1/2 z-30 w-full max-w-lg -translate-x-1/2 border-t border-orange-100 bg-white/95 px-3 py-2 shadow-[0_-10px_40px_rgba(15,23,42,0.08)] backdrop-blur-xl"><div className="flex gap-2 rounded-2xl bg-orange-50 p-2 ring-1 ring-orange-100"><input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && sendMessage()} className="flex-1 bg-transparent px-2 text-sm font-bold text-slate-900 outline-none placeholder:text-slate-400" placeholder="Scrie ce cauți..." /><button onClick={() => sendMessage()} disabled={!input.trim() || isLoading} className="grid h-10 w-10 place-items-center rounded-2xl bg-[#ff6b35] text-white shadow-lg disabled:opacity-40"><Send size={16} /></button></div></div>}
