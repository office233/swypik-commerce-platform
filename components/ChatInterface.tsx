...
export default function ChatInterface({ 
  ...
  return <main className={`min-h-screen ${THEME.classes.appBg}`}><div className={`relative mx-auto min-h-screen max-w-lg md:max-w-2xl lg:max-w-4xl xl:max-w-6xl app-container ${THEME.classes.pageBg}`}>{activeTab !== "feed" && <header className="sticky top-0 z-30 border-b border-[#E5E5E5] bg-white/95 px-4 py-3 backdrop-blur-xl safe-top"><div className="flex items-center justify-between"><button type="button" onClick={() => setActiveTab("home")} className="flex items-center gap-2"><h1 className="text-lg font-black text-[#0D0D0D]">Swypik</h1></button><button type="button" onClick={() => setActiveTab("cart")} className="relative rounded-xl bg-[#0D0D0D] p-2.5 text-white"><ShoppingCart size={18} />{cartCount > 0 && <span className="absolute -right-1 -top-1 grid h-5 w-5 place-items-center rounded-full bg-[#10A37F] text-[10px] font-black text-white">{cartCount}</span>}</button></div></header>}
