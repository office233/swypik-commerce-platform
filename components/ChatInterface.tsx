...
export default function ChatInterface() {
  ...
  async function loadFeed() { if (feedLoading) return; setFeedLoading(true); try { const data = await fetch("/api/products?mode=video&limit=200&sort=popular").then((r) => r.json()); setFeedProducts(data.products || []); } finally { setFeedLoading(false); } }
  const [touchStart, setTouchStart] = useState<{ x: number, y: number } | null>(null);

  async function loadMoreFeed() { if (feedLoading) return; setFeedLoading(true); try { const offset = feedProducts.length; const data = await fetch(`/api/products?mode=video&limit=200&offset=${offset}&sort=popular`).then((r) => r.json()); setFeedProducts((prev) => { const existing = new Set(prev.map((p) => p.id)); return [...prev, ...(data.products || []).filter((p: ChatProduct) => !existing.has(p.id))]; }); } finally { setFeedLoading(false); } }
